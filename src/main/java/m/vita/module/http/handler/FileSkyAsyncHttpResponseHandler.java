...
public abstract class FileJEBAsyncHttpResponseHandler extends JEBAsyncHttpResponseHandler {
    ...
    /**
     * Obtains new FileJEBAsyncHttpResponseHandler and stores response in passed file
     *
     * @param file File to store response within, must not be null
     */
    public FileJEBAsyncHttpResponseHandler(File file) {
        this(file, false);
    }
     ...
     */
    public FileJEBAsyncHttpResponseHandler(File file, boolean append) {
     ...
     */
    public FileJEBAsyncHttpResponseHandler(File file, boolean append, boolean renameTargetFileIfExists) {
     ...
     */
    public FileJEBAsyncHttpResponseHandler(File file, boolean append, boolean renameTargetFileIfExists, boolean usePoolThread) {
        super(usePoolThread);
        Utils.asserts(file != null, "File passed into FileJEBAsyncHttpResponseHandler constructor must not be null");
        if (!file.isDirectory() && !file.getParentFile().isDirectory()) {
            Utils.asserts(file.getParentFile().mkdirs(), "Cannot create parent directories for requested File location");
        }
        if (file.isDirectory()) {
            if (!file.mkdirs()) {
                JEBAsyncHttpClient.log.d(LOG_TAG, "Cannot create directories for requested Directory location, might not be a problem");
            }
        }
        this.file = file;
        this.append = append;
    ...
    /**
     * Obtains new FileJEBAsyncHttpResponseHandler against context with target being temporary file
     *
     * @param context Context, must not be null
     */
    public FileJEBAsyncHttpResponseHandler(Context context) {
        super();
    ...
    protected File getTemporaryFile(Context context) {
        ...
        try {
            ...
            JEBAsyncHttpClient.log.e(LOG_TAG, "Cannot create temporary file", e);
        }
        return null;
    }
        ...
        if (entity != null) {
            ...
            if (instream != null) {
                try {
                    ...
                    buffer.flush();
                    JEBAsyncHttpClient.silentCloseOutputStream(buffer);
                }
            }
        }
        return null;
