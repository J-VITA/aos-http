...
import m.vita.module.http.RequestParams;
...
import m.vita.module.http.util.Base64OutputStream;

public class JEBJsonStreamerEntity implements HttpEntity {
    private static final String LOG_TAG = "JEBJsonStreamerEntity";
    ...
    private static final Header HEADER_JSON_CONTENT =
            new BasicHeader(
                    JEBAsyncHttpClient.HEADER_CONTENT_TYPE,
                    RequestParams.APPLICATION_JSON);
    private static final Header HEADER_GZIP_ENCODING =
            new BasicHeader(
                    JEBAsyncHttpClient.HEADER_CONTENT_ENCODING,
                    JEBAsyncHttpClient.ENCODING_GZIP);
    ...
    public JEBJsonStreamerEntity(ResponseHandlerInterface progressHandler, boolean useGZipCompression, String elapsedField) {
    ...
    public void writeTo(final OutputStream out) throws IOException {
        ...
        if (0 < keysCount) {
                ...
                try {
                    ...
                    if (value == null) {
                        ...
                        if (isFileWrapper || value instanceof RequestParams.StreamWrapper) {
                            ...
                            os.write('}');
                        } else if (value instanceof JEBJsonValueInterface) {
                            os.write(((JEBJsonValueInterface) value).getEscapedJsonValue());
            ...
            JEBAsyncHttpClient.log.i(LOG_TAG, "Uploaded JSON in " + Math.floor(elapsedTime / 1000) + " seconds");
        }
        ...
        os.flush();
        JEBAsyncHttpClient.silentCloseOutputStream(os);
    }
    ...
    private void writeToFromStream(OutputStream os, RequestParams.StreamWrapper entry)
        ...
        JEBAsyncHttpClient.silentCloseOutputStream(bos);
        ...
        if (entry.autoClose) {
            ...
            JEBAsyncHttpClient.silentCloseInputStream(entry.inputStream);
        }
    }
    ...
    private void writeToFromFile(OutputStream os, RequestParams.FileWrapper wrapper)
        ...
        // Safely close the input stream.
        JEBAsyncHttpClient.silentCloseInputStream(in);
    }
